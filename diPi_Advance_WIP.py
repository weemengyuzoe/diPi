...
capture_press_time = None #time.time() when button first went down
BULB_HOLD_THRESHOLD = 1.0 #seconds of hold to enter bulb mode

# Bulb mode
bulb_start_time = 0.0 #when bulb exposure start
bulb_accumulator = None #float32 numpy array, running sum of frames
bulb_frame_count = 0 #number of frames stacked so far
bulb_blink_state = False #Led blink toggle
bulb_blink_last = 0.0 #last LED blink toggle time
BULB_BLINK_RATE = 0.4 #seconds per blink half-cycle
...
def draw_bulb_overlay(frame: np.ndarray, elapsed: float, n_frames: int) -> np.ndarray:
    """Overlay shown during an active bulb exposure."""
    out = frame.copy()
    # Dim red tint over the frame so the user knows bulb is active
    tint        = out.astype(np.float32)
    tint[:,:,2] = np.clip(tint[:,:,2] * 1.0 + 30, 0, 255)   # slight red channel boost (BGR)
    out         = tint.astype(np.uint8)
 
    mins = int(elapsed) // 60
    secs = int(elapsed) % 60
    put_text(out, f"BULB  {mins:02d}:{secs:02d}  ({n_frames} frames)",
             (10, 25), scale=0.7, thickness=2)
    put_text(out, f"Press capture again to stop & save", (10, 58))
    return out
...
# ── Input ─────────────────────────────────────────────────
def poll_inputs():
    global temp_dir, tint_dir, capture_press_time
    ...
    if focus_changed:
        focus["lens_position"] = max(
            FOCUS_MIN,
            min(FOCUS_MAX, focus["lens_position"])
        )
        set_lens_position(round(focus["lens_position"],2))


    # ───────────────── CAPTURE / BULB ─────────────────
    if BTN_CAPTURE.is_pressed:

        if capture_press_time is None:
            capture_press_time = now

        elif now - capture_press_time >= BULB_HOLD_THRESHOLD:
            capture_press_time = None
            return "bulb_start"

    else:
        if capture_press_time is not None:

            held = now - capture_press_time
            capture_press_time = None

            if held < BULB_HOLD_THRESHOLD:
                if now - last_action["capture"] > DEBOUNCE:
...
def apply_bulb_controls():
    """
    Lock AGC, AWB and colour gains at their current values for the
    duration of the bulb exposure so brightness/colour stay consistent
    across every stacked frame.
    """
    ...
    cam.set_controls({
        ...
        "AeEnable":           False,
        "AwbEnable":          False,
        # Lock the gains/shutter the AGC had settled on
        "ExposureTime":       metadata.get("ExposureTime",   33333),
...
def restore_auto_controls():
    """Re-enable AGC and AWB after bulb exposure ends."""
    cam.set_controls({
        "AeEnable":  True,
        "AwbEnable": True,
    })
...
def bulb_start():
    global STATE
    global bulb_start_time
    global bulb_accumulator
    global bulb_frame_count
    global bulb_blink_state
    global bulb_blink_last

    STATE = "bulb"

    bulb_start_time = time.time()

    bulb_accumulator = None
    bulb_frame_count = 0

    bulb_blink_state = True
    bulb_blink_last = time.time()

    apply_bulb_controls()

    led_set(red=True)

    print("Bulb mode started")
 
 
# ── Bulb: add frame ───────────────────────────────────────
 
def bulb_add_frame(frame: np.ndarray):
    """Stack a new preview frame into the running accumulator."""
    global bulb_accumulator, bulb_frame_count
    f32 = frame.astype(np.float32)
    if bulb_accumulator is None:
        bulb_accumulator = f32
    else:
        bulb_accumulator += f32
    bulb_frame_count += 1
...
def bulb_stop(sw, sh):
    global STATE, frozen_frame, freeze_until, capture_green_until
 
    elapsed = time.time() - bulb_start_time
 
    if bulb_accumulator is None or bulb_frame_count == 0:
        print("  Bulb: no frames captured, discarding")
        STATE = "camera"
        restore_auto_controls()
        leds_off()
        return
 
    # Average the stacked frames and apply grade
    averaged    = np.clip(bulb_accumulator / bulb_frame_count, 0, 255).astype(np.uint8)
    graded_bulb = apply_grade(averaged)
 
    filename = time.strftime("bulb_%Y%m%d_%H%M%S.jpg")
    filepath = os.path.join(SAVE_DIR, filename)
    cv2.imwrite(filepath, graded_bulb, [cv2.IMWRITE_JPEG_QUALITY, 97])
    print(f"  Bulb saved -> {filepath}  "
          f"({bulb_frame_count} frames, {elapsed:.1f}s)")
 
    restore_auto_controls()
    STATE = "camera"
 
    # Green LED confirmation
    capture_green_until = time.time() + CAPTURE_GREEN_DURATION
    led_set(green=True)
 
    # Freeze display showing the result
    frozen_frame = fit_to_screen(graded_bulb, sw, sh)
    put_text(frozen_frame,
             f"BULB SAVED  {int(elapsed)}s / {bulb_frame_count}f",
             (sw // 2 - 120, sh // 2), scale=0.9, thickness=2)
    freeze_until = time.time() + 4.0
    cv2.imshow("Camera", frozen_frame)
...
while True:
    ...
    if STATE == "camera":
        ...
        if now < freeze_until and frozen_frame is not None:
            ...
            if action == "capture":
                ...
                print(f" Saved -> {filepath} " f"({full_frame.shape[1]}x{full_frame.shape[0]})")
                print(f" Grade: temp={grade['temperature']:+.1f} " f"focus={focus['lens_position']:.1f} dpt " f"lux={current_lux:.0f}")
                capture_green_until = now + CAPTURE_GREEN_DURATION
                led_set(green=True)
                frozen_frame = fit_to_screen(graded_full, SW, SH)
                
                put_text(frozen_frame, "SAVED!", (SW // 2 - 40, SH // 2), scale=1.2, thickness=2)
    ...
    # ───────────────── BULB MODE ─────────────────
        ...
        frame = cam.capture_array()
        bulb_add_frame(frame)

        if bulb_accumulator is not None and bulb_frame_count > 0:
            live = np.clip(
                bulb_accumulator / bulb_frame_count, 0, 255
            ).astype(np.uint8)

            live = apply_grade(live)
            live = fit_to_screen(live, SW, SH)

            elapsed = now - bulb_start_time
            live = draw_bulb_overlay(live, elapsed, bulb_frame_count)

            cv2.imshow(WINDOW, live)

        # LED blinking
        if now - bulb_blink_last >= BULB_BLINK_RATE:
            bulb_blink_last = now
            bulb_blink_state = not bulb_blink_state
            led_set(red=bulb_blink_state)
